...
test('Prueba13: Probar seleccionar un elemento de un menu', async t => {
    ...
    //.expect(pagina.menu.javascript.selectedIndex).eql(1);

});

test('Prueba14: Dar click en un checkbox para activar un area de text', async t =>{
    await t
    .click(pagina.triedCheckbox)
    .typeText(pagina.textArea,'Primera linea')
    .typeText(pagina.textArea,'/nSegunda linea')
    .typeText(pagina.textArea,'/nTercera linea')
    .selectTextAreaContent(pagna.textArea,1,5)
    .pressKey('delete')
...
test('Prueba 15: manejar un slider bar', async t =>{
    await t
    .click(pagina.triedCheckbox)
    .dragToElement(pagina.slider.handle,pagina.slider.tick.withText)
});
